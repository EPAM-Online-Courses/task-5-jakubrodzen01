...
public class GameLobby {
    ...
    public List<Hero> getHeroesFromTown(Town town) {
        List<Hero> list = playableTownsWithHeroesList.get(town);
        if (list != null)
            return list; //zwróć listę bohaterów z tego miasta.
        else
    ...
    public Map<Town, List<Hero>> mapHeroesToStartingTowns(List<Town> availableTowns, Set<Hero> availableHeroes) {
        Map<Town, List<Hero>> startingHeroTowns = new TreeMap<>();
        for (Town town : availableTowns) {
            ...
            startingHeroTowns.put(town, tmp);
        }
        return startingHeroTowns;
    }
